...
// --- Types (using JSDoc for prop types) ---
/**
 * @typedef {'user' | 'bot' | 'auditor'} Role
 * @typedef {{
 *   id: string;
 *   role: Role;
 *   text: string;
 ...
 *     userIncludes?: string[];
 *     botIncludes?: string[];
 *     daysSinceOrderOver?: number;
 *   };
 *   action: 'stop' | 'interject' | 'modify';
 *   onViolationGuidance: string;
 * }} Rule
 * @typedef {{
 *   name: string;
 *   version: string;
 ...
 *     interjectMinConfidence: number;
 *   };
 *   rules: Rule[];
 *   supportProtocols: string[];
 * }} SchemaDoc
 * @typedef {{
 *   outcome: 'allow' | 'stop' | 'interject-modify' | 'interject-ask-user';
 *   confidence: number;
 *   triggeredRules: Rule[];
 *   rationale: string[];
  ...
  rules: [
    {
      ...
      description: 'If the user requests a full refund beyond 30 days, the bot must not process it automatically; offer repair/credit or escalate.',
      severity: 5,
      match: { userIncludes: ['refund', 'return'], daysSinceOrderOver: 30 },
      action: 'interject',
      onViolationGuidance: 'Explain refund window, offer repair or 30% store credit, or escalate to a human specialist for exceptions.',
    },
    {
      id: 'R-002',
      title: 'Avoid promising outcomes outside policy',
      description: 'Do not promise a full refund or replacement when policy conditions are not met.',
      severity: 4,
      match: { botIncludes: ['processed a full refund', "absolutely! i've processed"] },
      action: 'modify',
      onViolationGuidance: 'Replace promises with compliant alternatives and set expectations per policy.',
    },
    {
      id: 'R-003',
      title: 'No sensitive data collection',
      description: 'Do not ask for or reveal full card numbers, SSNs, or passwords. Card last4 is permitted only if user provides it first.',
      severity: 4,
      match: { botIncludes: ['full card number', 'security number', 'password'] },
      action: 'stop',
      onViolationGuidance: 'Mask or avoid sensitive data. Ask for non-sensitive identifiers or use verified account channel.',
    },
    {
      id: 'R-004',
      title: 'Escalate high-value exceptions',
      description: 'If order value > $100 AND outside standard policy, escalate to a human.',
      severity: 3,
      match: { userIncludes: ['$', 'dollars', 'order'] },
      action: 'interject',
      onViolationGuidance: 'Offer to bring a specialist in for out-of-policy high-value cases.',
    },
  ],
};
...
function audit(conversation, schema) {
  ...
  const rationale = triggered.map((r) => `${r.id}: ${r.title} (severity ${r.severity})`);
 ...
 * }} props
 */
export default function UserAuditModal({ conversation: initialConversation, userName, onClose }) {
        ...
        <div className="flex-1">
          ...
          <div
            className={`whitespace-pre-wrap leading-6 ${
              isAuditor ? 'bg-amber-50 border border-amber-200' : 'bg-gray-50 border border-gray-200'
            } p-3 rounded-xl`}
          >
            {m.text}
          </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-gray-50 rounded-2xl shadow-2xl w-full max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 border-b border-gray-200 bg-white flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight">AI Audit: {userName}</h2>
            <p className="text-gray-600 text-sm">Real-time policy enforcement review</p>
          </div>
          <div className="flex items-center gap-3">
            <Badge tone={outcomeTone[decision.outcome].tone}>{outcomeTone[decision.outcome].label}</Badge>
            <button onClick={resetDemo} className="px-3 py-2 rounded-xl border border-gray-300 hover:bg-gray-50 bg-white text-sm">
              Reset
            </button>
            <button onClick={onClose} className="px-3 py-2 rounded-xl bg-gray-900 text-white text-sm hover:bg-gray-800">
              Close
            </button>
          </div>
        </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium border ${
                activeTab === 'conversation' ? 'bg-white border-gray-300 shadow-sm' : 'bg-gray-100 border-gray-200 hover:bg-white'
              }`}
            >
              Conversation
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium border ${
                activeTab === 'schema' ? 'bg-white border-gray-300 shadow-sm' : 'bg-gray-100 border-gray-200 hover:bg-white'
              }`}
            >
              Schema
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium border ${
                activeTab === 'auditlog' ? 'bg-white border-gray-300 shadow-sm' : 'bg-gray-100 border-gray-200 hover:bg-white'
              }`}
            >
              Audit Log
            </button>
        ...
        <div className="flex-1 overflow-auto p-6">
                      ...
                      <div className="mt-2">
                        <div className="p-4 rounded-xl border border-amber-200 bg-amber-50">
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            <button onClick={applySuggestion} className="px-3 py-2 rounded-xl bg-black text-white text-sm">
                              Approve & Send modified
                            </button>
                            <button onClick={requestHuman} className="px-3 py-2 rounded-xl border border-gray-300 bg-white text-sm">
                              Stop & Request human
                            </button>
                            <button onClick={allowOriginal} className="px-3 py-2 rounded-xl border border-gray-300 bg-white text-sm">
                              Override & Send original
                            </button>
                          </div>
                        </div>
                      </div>
              ...
              <div className="col-span-3">
                ...
                <Card className="p-4">
                  ...
                  {parsedSchema ? (
                    <div className="text-sm text-gray-700">
                      <div className="mb-2">
                        <span className="font-medium">Name:</span> {parsedSchema.name}
                      </div>
                      <div className="mb-2">
                        <span className="font-medium">Version:</span> {parsedSchema.version}
                      </div>
                      <div className="mb-2">
                        <div className="font-medium mb-1">Protocols</div>
                        <ul className="list-disc pl-5 space-y-1">
                          {parsedSchema.supportProtocols.slice(0, 3).map((p, i) => (
                            <li key={i}>{p}</li>
                          ))}
                        </ul>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <div className="text-sm text-gray-600">
                        Source this from your support protocol documentation. Edit below and return to the Conversation tab to re-run the auditor.
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button
                      ...
                      <button className="px-3 py-2 rounded-xl bg-black text-white text-sm" onClick={() => setActiveTab('conversation')}>
                        Re-run audit
                      </button>
                    </div>
                  </div>
              ...
              <div className="col-span-4">
                <Card className="p-4">
                  ...
                  <ul className="list-disc pl-5 text-sm text-gray-700 space-y-2">
                    <li>
                      Map each SOP rule to an <span className="font-medium">id</span>, <span className="font-medium">title</span>,{' '}
                      <span className="font-medium">severity</span>, and <span className="font-medium">action</span> (stop/interject/modify).
                    </li>
                    <li>
                      Use <span className="font-medium">match.userIncludes</span> and <span className="font-medium">match.botIncludes</span> for quick
                      keyword demos. Real systems would use classifiers + semantic patterns.
                    </li>
                    <li>
                      Tune <span className="font-medium">thresholds</span> to change when the auditor interrupts the chatbot.
                    </li>
                    <li>
                      Provide <span className="font-medium">onViolationGuidance</span> so the auditor can draft a compliant reply automatically.
                    </li>
                  </ul>
                </Card>
              </div>
          ...
          {activeTab === 'auditlog' && (
            <Card className="p-4">
              ...
              <div className="text-sm text-gray-700">
                This demo shows one interjection. In a full app, you'd see a chronological list of decisions with diffs, assignee, and resolution status.
              </div>
            </Card>
          )}
        </div>
        ...
        <div className="px-6 py-3 border-t border-gray-200 bg-white">
          <div className="text-xs text-gray-500">Demo-only logic. In production, plug your realtime chat stream + policy engine here.</div>
        </div>
      </div>
    </div>
  );
